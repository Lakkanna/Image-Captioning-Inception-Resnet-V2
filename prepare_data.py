...
def preprocessing():
	...
	for i in range(len(image_captions)-1):
		...
		id_capt[0] = id_capt[0][:len(id_capt[0])-2] 	# to remove off the #0,#1,#2,#3,#4 from the tokens file
		try:
	...
	print("train images processed: trainimgs.txt")
	...
	print("test images processed: testimgs.txt")
